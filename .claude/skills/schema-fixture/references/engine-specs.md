...
|---|---|---|
| `test/schema/engine.primitives.spec.ts` | One Zod *leaf* type per case — string, number, boolean, null, bigint, date, literal, enum. | Snippet is a single `z.X()` call with no chained operators. |
