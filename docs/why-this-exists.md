...
If you need to serve OpenAPI 3.0 to a client that can't speak 3.1, run a downgrade pass _after_ `applyZodNest` — there are good standalone tools for it. `zod-nest` itself emits 3.1 or 3.2 only — both are JSON Schema 2020-12 based, so neither needs that rewriting.
