...
- **Multipart file uploads** — `zod-nest/express` and `zod-nest/fastify` subpaths with per-parser file schemas (`multerMemoryFile`, `multerDiskFile`, `fastifyMultipartFile`) and `@ZodMultipart` / `@ZodUploadedFile` / `@ZodUploadedFiles` / `@ZodMultipartBody`, replacing the hand-wired `@ApiConsumes` + `@ApiBody` + `ParseFilePipe` trio. See [`docs/file-uploads.md`](docs/file-uploads.md).
...
**"`createZodDto(mySchema)` fails to compile with TS2509 — my response schema is a `z.discriminatedUnion` / `z.union` / `z.intersection`."**
A schema whose `z.infer<>` is a TS union can't be a class base, so `class Dto extends createZodDto(schema) {}` errors with _"Base constructor return type … is not an object type"_ (TS2509). For a **response**, you no longer need a DTO: pass the schema straight to `@ZodResponse({ type: mySchema })`. It's normalised to `createZodDto(schema).Output` internally — output IO, registered under the schema's `.meta({ id })` — replacing the old `registerSchema(schema)` + hand-written `@ApiResponse({ content: { … { schema: { $ref } } } })` workaround. Raw schemas also work in the array (`[schema]`) and tuple forms, and mix with DTOs. For a **request body**, use `@ZodBody(schema)` + `@Body(new ZodValidationPipe(schema))` instead. See [`docs/responses.md → "Passing a raw schema"`](docs/responses.md) and [`docs/recipes/intersection-with-union.md`](docs/recipes/intersection-with-union.md).

**"My multipart upload endpoint throws `ZodNestUnrepresentableError`, or its Swagger try-it-out form renders one stub field instead of a file picker."**
Two separate causes. The error means a `z.custom` / `z.instanceof` file field emitted `{}` — the shipped `FileSchema` / `BlobSchema` / `BufferSchema` presets only cover class-based runtime types, and neither multer nor `@fastify/multipart` produces a class. Use the platform helper (`multerMemoryFile()` from `zod-nest/express`, `fastifyMultipartFile()` from `zod-nest/fastify`) instead of hand-rolling a `z.custom`. The stub-field rendering is Swagger UI refusing to follow a `$ref` for a `multipart/form-data` body; `@ZodMultipart(shape)` emits the body inline for exactly this reason, and also sets `@ApiConsumes` for you.

Note that a multer file never reaches `@Body()` — `FileInterceptor` puts it on `req.file`, so a DTO listing the file alongside its text fields validates only in the document, not at runtime. `@ZodUploadedFile(name)` reads `req.file` and `@ZodMultipartBody()` reads the text fields, both against the one shape declared on `@ZodMultipart`. See [`docs/file-uploads.md`](docs/file-uploads.md) and [`docs/recipes/multipart-uploads.md`](docs/recipes/multipart-uploads.md).

**"I get `ZodNestDocumentError: DANGLING_REF` at boot."**
`applyZodNest` validates the final `$ref` graph. A dangling ref means a DTO is referenced from the doc but the registry doesn't know about it — typically a typo'd `.meta({ id })`, two `ZodNestRegistry` instances being used in the same app, or a user-supplied pre-pass that injected a ref to a non-existent component. Earlier versions also raised it when a named schema was nested through a clone — `Base.describe(...)`, `Base.refine(...)`, `z.compile(Base)`; those now resolve to the named ancestor. See [`docs/schema-identity.md`](docs/schema-identity.md). The error message lists every offending ref with a hint from the collected-usage table. Schemas used only as `extend()` parents auto-register since 1.6 (`extend()` calls `registerSchema` on parent + result); for other named-but-DTO-less references, register the schema directly with `registerSchema(schema)` or wrap it in `createZodDto`.

**"I get `ZodNestError: A schema referenced from inside its own emission has no id` after upgrading."**
A cycle reaches back to the root schema — mutual recursion, or a self-referencing getter — and that root has no `.meta({ id })`. The reference needs a `#/components/schemas/<id>` target, and an unnamed root has no component entry to point at. Earlier versions emitted anyway and produced a `$ref` that silently resolved to the **wrong** schema (the def that happened to contain the reference), so a doc that "worked" before was already describing the wrong shape. Give every schema in the cycle an id — `.meta({ id: 'Root' })` on the root is usually the only change needed. `strict: false` downgrades the throw, but leaves the reference unresolvable. See [`docs/exceptions.md`](docs/exceptions.md#unresolvable-self-reference-on-an-unnamed-root) and [`docs/recipes/recursive-schemas.md`](docs/recipes/recursive-schemas.md).
